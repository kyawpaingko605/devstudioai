...
import devstudioai.android.support.AndroidFeatureDependencies
...
import devstudioai.model.module
import devstudioai.ui.backend.UiBuildFeature
import devstudioai.ui.backend.UiBuildFeatures
import devstudioai.ui.backend.UiConfigField
import devstudioai.ui.backend.UiConfigResult
import devstudioai.ui.backend.UiFacetConfig
import devstudioai.ui.backend.UiMissingProguardFile
import devstudioai.ui.backend.UiModuleConfig
import devstudioai.ui.backend.UiModuleConfigEdit
import devstudioai.ui.backend.UiModuleRef
import devstudioai.ui.backend.UiModuleTypeOption
import devstudioai.ui.backend.UiRunConfig
import devstudioai.ui.backend.UiSourceSetInfo
 ...
 * [devstudioai.core.IdeServices]. Reaches shared infrastructure (model, prefs) through [EngineContext]; enabling
...
internal class ModuleService(private val ctx: EngineContext) {
    ...
    fun updateModuleConfig(moduleName: String, edit: UiModuleConfigEdit): UiConfigResult {
        ...
        val facets = ArrayList<devstudioai.model.Facet>()
        for ((table, values) in edit.facetValues) {
            val facet = ctx.store.facetCodecs.decode(devstudioai.model.impl.FacetData(table, values))
    ...
    fun missingProguardFiles(moduleName: String): List<UiMissingProguardFile> {
        ...
        fun consider(
            bt: devstudioai.android.support.BuildType, entries: List<String>, consumer: Boolean
        ) {
            for (e in entries) {
                if (devstudioai.android.support.DefaultProguardFiles.isDefault(e)) continue
                val p = Paths.get(e)
    ...
    fun createProguardFile(moduleName: String, entry: String): Path? {
        ...
        if (devstudioai.android.support.DefaultProguardFiles.isDefault(entry)) return null
