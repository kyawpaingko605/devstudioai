...
import devstudioai.model.ClasspathSnapshot
...
enum class BackendCapability {
    ...
    POSTFIX,            // contributes/handles postfix templates (devstudioai.lang.postfix)
...
interface SourceAnalyzer {
     ...
     * or scope-driven keyword logic) as first-class engine contributors. See `devstudioai.lang.completion`.
     */
    fun completionContributions(): List<devstudioai.lang.completion.CompletionContribution> = emptyList()

    /** Parameter-info popup; null if !capabilities.contains(SIGNATURE_HELP). See the signature SPI. */
    val signatureHelp: devstudioai.lang.signature.SignatureHelpService?
        get() = null

    /** Type-aware editor coloring; null if !capabilities.contains(SEMANTIC_HIGHLIGHT). See the highlight SPI. */
    val semanticHighlighter: devstudioai.lang.highlight.SemanticHighlightService?
        get() = null

    /** Code-folding regions; null if !capabilities.contains(CODE_FOLDING). See the folding SPI. */
    val folding: devstudioai.lang.folding.FoldingService?
        get() = null

    /** Code reformatting; null if !capabilities.contains(FORMAT). See the formatting SPI. */
    val formatting: devstudioai.lang.formatting.FormattingService?
        get() = null
     ...
     */
    fun fileStructure(file: VirtualFile, text: CharSequence): List<devstudioai.lang.resolve.StructureItem> = emptyList()
     ...
     */
    fun quickDoc(file: VirtualFile, text: CharSequence, offset: Int): devstudioai.lang.resolve.QuickDocInfo? = null
    ...
    fun expectedTypeAt(file: VirtualFile, offset: Int): devstudioai.lang.resolve.TypeRef?
     ...
     */
    fun resolveType(node: DomNode): devstudioai.lang.resolve.TypeRef? = null
}
