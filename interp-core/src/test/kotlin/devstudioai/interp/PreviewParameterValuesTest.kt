...
class PreviewParameterValuesTest {
    ...
    fun readsLibraryProviderValues() {
        val values = interpreter.previewParameterValues(null, "devstudioai.interp.SampleStringProvider", Int.MAX_VALUE)
        assertEquals(listOf("alpha", "beta", "gamma"), values)
    }

    @Test
    fun respectsLimit() {
        val values = interpreter.previewParameterValues(null, "devstudioai.interp.SampleStringProvider", 2)
