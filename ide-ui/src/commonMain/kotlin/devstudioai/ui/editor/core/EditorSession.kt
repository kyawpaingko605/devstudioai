package devstudioai.ui.editor.core
...
import devstudioai.ui.editor.CodeLanguage
import devstudioai.ui.editor.shiftComposePreviews
import devstudioai.ui.editor.shiftDiagnostics
import devstudioai.ui.editor.shiftFoldRegions
import devstudioai.ui.editor.shiftInlayHints
import devstudioai.ui.editor.shiftSemanticTokens
...
class EditorSession(
     ...
     * ([devstudioai.ui.editor.shiftInlayHints]) so the phantom text tracks its anchor between debounced daemon
     ...
     * ([devstudioai.ui.editor.shiftFoldRegions]) so a collapsed fold keeps covering its block while typing; the
     ...
     */
    var foldRegions by mutableStateOf<List<devstudioai.ui.editor.folding.FoldRegion>>(emptyList())
        private set
    ...
    private var foldModelCache: devstudioai.ui.editor.folding.FoldModel? = null
    private var foldModelDoc: EditorDocument? = null
    private var foldModelRegions: List<devstudioai.ui.editor.folding.FoldRegion>? = null

    /** The current folding projection (document ⇄ visual-row mapping + composites), rebuilt only when the
     *  regions or the document change. [devstudioai.ui.editor.folding.FoldModel.EMPTY]-equivalent when nothing folds. */
    val foldModel: devstudioai.ui.editor.folding.FoldModel
        get() {
            val d = doc; val r = foldRegions
            if (foldModelCache == null || foldModelDoc !== d || foldModelRegions !== r) {
                foldModelCache = devstudioai.ui.editor.folding.FoldModel.build(d, r)
     ...
     */
    fun applyCodeFolds(fresh: List<devstudioai.ui.editor.folding.FoldRegion>) {
