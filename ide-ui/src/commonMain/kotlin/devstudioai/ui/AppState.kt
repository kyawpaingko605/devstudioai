...
import devstudioai.ui.backend.UiSourceRootRole
import devstudioai.ui.backend.UiNewFileTemplate
import devstudioai.ui.editor.core.EditorSession
import devstudioai.ui.editor.languageFor
...
class OpenFile(val path: String, val name: String, initial: String) {
    ...
    var viewMode by mutableStateOf(
        if (devstudioai.ui.editor.preview.previewKindOf(path) == devstudioai.ui.editor.preview.PreviewKind.BITMAP)
...
class IdeUiState(val backend: IdeBackend, val composePreviewHost: ComposePreviewHost? = null) {
    ...
    fun applySettings(s: devstudioai.ui.backend.UiSettings) {
    ...
    fun tabsSnapshot(): devstudioai.ui.backend.UiOpenTabs =
        devstudioai.ui.backend.UiOpenTabs(openFiles.map { it.path }, activeIndex)
