package devstudioai.ui.editor.preview
...
import devstudioai.preview.FontMetrics
import devstudioai.preview.PreviewResources
...
import devstudioai.preview.RGraphics
import devstudioai.preview.RImage
import devstudioai.preview.RPaint
import devstudioai.preview.RPath
...
internal class ComposePaint : RPaint {
    ...
    override var gradient: devstudioai.preview.Gradient? = null
}
...
internal class ComposeRCanvas(
...
) : RCanvas, devstudioai.preview.NativeCanvasHolder {
    ...
    private fun paintFor(p: RPaint, l: Float, t: Float, r: Float, b: Float): Paint = paint(p).also {
        ...
        it.shader = when (g.type) {
            devstudioai.preview.GradientType.RADIAL -> androidx.compose.ui.graphics.RadialGradientShader(
                center = Offset(l + (r - l) * g.centerX, t + (b - t) * g.centerY),
                radius = minOf(r - l, b - t) * g.radiusFraction,
                colors = colors,
            )
            devstudioai.preview.GradientType.SWEEP -> androidx.compose.ui.graphics.SweepGradientShader(
