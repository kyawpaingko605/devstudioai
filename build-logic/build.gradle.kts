...
// which buildSrc cannot do. Currently: `devstudioai.kotlinc-art`, the Kotlin-compiler-on-ART ASM
// instrumentation.
plugins {
    `kotlin-dsl`
}
...
// The `devstudioai.kotlinc-art` plugin id → :ide-android applies it with one line.
gradlePlugin {
    plugins {
        create("kotlincArt") {
            id = "devstudioai.kotlinc-art"
            implementationClass = "devstudioai.build.kotlinc.KotlincArtPlugin"
        }
    }
}
