...
class RealViewResult(
     ...
     *  when the runtime didn't snapshot it. Carried up to [devstudioai.preview.LayoutPreviewResult.viewTree] so
     *  the Preview view can show the hierarchy + a tap-to-inspect panel over the real-view render. */
    val viewTree: devstudioai.preview.PreviewViewNode? = null,
)
 ...
 * (`devstudioai.preview.realview`). Returns null when no runtime is configured (→ the owned-rendering path).
 */
