...
object CardRenderer : Renderer {
    ...
    override fun draw(node: RenderNode, canvas: RCanvas, ctx: RenderContext) {
        ...
        for (c in node.children) if (c.props.visibility != devstudioai.preview.Props.GONE) c.renderer.draw(c, canvas, ctx)
    }
}

/** Registers Material + AppCompat tag aliases onto an existing registry (resolved by simple name). */
fun devstudioai.preview.RendererRegistry.withMaterial(): devstudioai.preview.RendererRegistry = this
