...
|---|---|---|
| `platform-core` | `devstudioai.platform` | Extension registry, message bus, model read/write lock, activities/progress, disposer, content hashing, plugin ids. |
| `vfs-api` | `devstudioai.vfs` | `VirtualFile`, `VirtualFileSystem`, file events, listeners. |
| `project-model-api` | `devstudioai.model`, `.graph` | Workspace/Project/Module/SourceSet/ContentRoot, order entries + scopes, classpath snapshots, library/SDK tables, module types, variants, facets, transactions, the project/module graphs, file-icon SPI. |
| `build-api` | `devstudioai.build` | `BuildSystem` SPI; the generic incremental task engine contracts (`Task`/`TaskInputs`/`TaskOutputs`/`TaskGraph`/`TaskExecutor`). |
| `language-api` | `devstudioai.lang`, `.dom`, `.incremental`, `.resolve`, `.completion` | `LanguageBackend` SPI, source analyzer/compiler contracts, the backend-neutral DOM, incremental parsing, symbol/scope/type resolution, code completion. |
| `deps-api` | `devstudioai.deps` | Dependency resolution SPI (Maven coordinates → jars/aars, conflict policy). |
| `index-api` | `devstudioai.index` | Indexing SPI: index extensions, index service, shared value types. |
| `analysis-api` | `devstudioai.analysis` | Diagnostics/analyzer/quick-fix SPI: one diagnostic model and one pipeline; compiler errors and analyzer findings merge into the same stream. |
| `block-api` | `devstudioai.block` | Projectional (block) editor SPI: block tree, block mappings, block edits, the projection service. |
