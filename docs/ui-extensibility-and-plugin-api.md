...
```kotlin
package devstudioai.plugin.action
...
*Delivered (installment 1):* The **UI-side action registry** in `devstudioai.ui.ext` (`UiActionHost`,
...
*Delivered (installment 2):* the **Compose-bearing registries** in `devstudioai.ui.ext` — `ToolWindowRegistry`
...
*Stage 2 (done):* `IdeServicesBackend` is split into 13 per-service impl classes (`devstudioai.core.backend.*`)
