...
`CompletionEngine` (`ide-core`, `devstudioai.core.completion`). The engine:
...
```kotlin
import devstudioai.lang.completion.*
...
Target a position with an `ElementPattern<DomNode>` over the neutral DOM (`devstudioai.lang.patterns`), so one
...
See `devstudioai.lang.completion.Contributor`, `devstudioai.lang.patterns.Patterns`,
`devstudioai.lang.postfix.Postfix`, and `devstudioai.core.completion.CompletionEngine`.
