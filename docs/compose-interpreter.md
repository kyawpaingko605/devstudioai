...
`:ide-android` (`devstudioai.android.spike.ComposeAbiSpikeTest`) validates the riskiest assumption before any
...
The Compose bridge lives as the `devstudioai.interp.compose` package inside `:ide-android` (where the real
...
`devstudioai.android.spike.ComposeAbiSpikeTest` (instrumented; runs on a device/emulator) is that spike. Like the
...
./gradlew :ide-android:connectedDebugAndroidTest \
  -Pandroid.testInstrumentationRunnerArguments.class=devstudioai.android.spike.ComposeAbiSpikeTest
...
1. ✅ ABI spike green on device (retires risk #2) — `devstudioai.android.spike.ComposeAbiSpikeTest`.
2. ✅ `ResolvedTree` contract + a resolver skeleton for a starter subset — `devstudioai.lang.kotlin.interp`
...
3. ✅ Plain-Kotlin interpreter (no Compose) over `ResolvedTree` — the new `:interp-core` module
   (`devstudioai.interp.Interpreter`), verified by `InterpreterTest`. Executes constants, locals/params, blocks,
...
4. ✅ Reflective dispatch + the Compose bridge.
   - **4a** — `interp-core` gained a `Dispatcher` seam and a default `ReflectiveDispatcher` (member calls via
     ...
     precision.
   - **4b** — `devstudioai.interp.compose.ComposeDispatcher` (in `:ide-android`) threads a real `Composer` into
...
5. ✅ Granular recomposition + state. `interp-core` gained a `ComposableInvoker` seam (the interpreter runs a
   ...
   dependency). `devstudioai.interp.compose.ComposeRuntime` (in `:ide-android`) implements the invoker: it opens
