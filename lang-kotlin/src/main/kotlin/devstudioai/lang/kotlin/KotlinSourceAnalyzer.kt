...
import devstudioai.lang.dom.Diagnostic
...
import devstudioai.lang.kotlin.completion.KotlinCompletion
import devstudioai.lang.kotlin.completion.KotlinCompletionItems
import devstudioai.lang.kotlin.parse.KotlinDomNode
import devstudioai.lang.kotlin.parse.KotlinIncrementalParser
import devstudioai.lang.kotlin.parse.KotlinParsedFile
import devstudioai.lang.kotlin.resolve.KotlinResolver
import devstudioai.lang.kotlin.symbols.KotlinSymbolService
import devstudioai.lang.resolve.ResolveResult
import devstudioai.lang.resolve.Scope
import devstudioai.lang.resolve.Symbol
import devstudioai.lang.resolve.SymbolFilter
...
import devstudioai.lang.kotlin.symbols.KotlinSymbol
 ...
 * parse). Mirrors [devstudioai.lang.jdt.JdtSourceAnalyzer]: built from a model-derived [CompilationContext],
...
class KotlinSourceAnalyzer(ctx: CompilationContext) : SourceAnalyzer, Disposable {
    ...
    @Volatile
    var syntheticClassProvider: () -> List<devstudioai.lang.synthetic.SyntheticClass> = { emptyList() }
    ...
    @Volatile
    var sourceDocProvider: devstudioai.lang.resolve.SourceDocProvider = devstudioai.lang.resolve.SourceDocProvider.NONE
    ...
    private fun syncFocal(parsed: KotlinParsedFile) {
        runCatching {
            service.syncFocal(parsed.file.path, parsed.ktFile.text.hashCode()) {
                devstudioai.lang.kotlin.symbols.SourceIndexBuilder.extractFrom(parsed.ktFile, parsed, parsed.file.path)
            }
        }
    }
    ...
    private class CachesEntry(val parsed: KotlinParsedFile, val caches: devstudioai.lang.kotlin.resolve.KotlinResolverCaches)
    ...
    private fun freshCachesFor(parsed: KotlinParsedFile): devstudioai.lang.kotlin.resolve.KotlinResolverCaches {
        val c = devstudioai.lang.kotlin.resolve.KotlinResolverCaches()
    ...
    private fun reuseCachesFor(parsed: KotlinParsedFile): devstudioai.lang.kotlin.resolve.KotlinResolverCaches {
    ...
    override fun completionContributions(): List<devstudioai.lang.completion.CompletionContribution> =
        listOf(devstudioai.lang.completion.CompletionContribution(completionContributor))

    override val inlayHints: devstudioai.lang.hints.InlayHintService by lazy {
    ...
    override val signatureHelp: devstudioai.lang.signature.SignatureHelpService by lazy {
        KotlinSignatureHelpService(service) { refreshOverlay() }
    }

    override val semanticHighlighter: devstudioai.lang.highlight.SemanticHighlightService by lazy {
    ...
    override val folding: devstudioai.lang.folding.FoldingService by lazy {
    ...
    override val formatting: devstudioai.lang.formatting.FormattingService = KotlinFormatter()
    ...
    private val previewLowering by lazy { devstudioai.lang.kotlin.interp.KotlinPreviewLowering(service, ::reuseCachesFor) }

    /** The `@Preview @Composable` functions in [file]'s last parse — the editor's preview targets. */
    fun composePreviews(file: VirtualFile): List<devstudioai.lang.kotlin.interp.PreviewInfo> =
     ...
     *  [devstudioai.lang.kotlin.interp.KotlinPreviewLowering.hasSyntaxErrors] for why. */
    ...
    /** Lower every top-level function in [file] to a [devstudioai.lang.kotlin.interp.ResolvedFunction], keyed
     *  `"name/arity"` — the program the interpreter runs a preview against (same-file composables included). */
    fun lowerFile(file: VirtualFile): Map<String, devstudioai.lang.kotlin.interp.ResolvedFunction> =
        lastByFile[file.path]?.let { previewLowering.program(it) } ?: emptyMap()

    /** Lower every source class/object/enum in [file] to a [devstudioai.lang.kotlin.interp.ResolvedClass] — the
     *  project-source types a preview's program may construct or reference. */
    fun lowerFileClasses(file: VirtualFile): List<devstudioai.lang.kotlin.interp.ResolvedClass> =
     ...
     *  [file] hasn't been parsed. See [devstudioai.lang.kotlin.interp.KotlinPreviewLowering.crossFileModel]. */
    fun lowerFileWithDeps(file: VirtualFile): devstudioai.lang.kotlin.interp.PreviewModel? =
    ...
    fun lowerFileWithDeps(
        file: VirtualFile, provider: devstudioai.lang.kotlin.interp.PreviewDeclProvider,
    ...
    fun loweredFile(pf: KotlinSymbolService.PreviewSourceFile): devstudioai.lang.kotlin.interp.PreviewFileModel? =
    ...
    override fun fileStructure(file: VirtualFile, text: CharSequence): List<devstudioai.lang.resolve.StructureItem> {
        val ktFile = KotlinParserHost.parse(file.name, text)
        val out = ArrayList<devstudioai.lang.resolve.StructureItem>()
        for (d in ktFile.declarations) collectStructure(d, 0, out)
        return out
    }

    private fun collectStructure(decl: KtDeclaration, depth: Int, out: MutableList<devstudioai.lang.resolve.StructureItem>) {
        when (decl) {
            is KtEnumEntry -> addStructure(decl.name, null, devstudioai.lang.resolve.SymbolKind.ENUM_CONSTANT, decl, depth, out)
            is KtClassOrObject -> {
                val kind = when {
                    decl is KtClass && decl.isInterface() -> devstudioai.lang.resolve.SymbolKind.INTERFACE
                    decl is KtClass && decl.isEnum() -> devstudioai.lang.resolve.SymbolKind.ENUM
                    decl is KtClass && decl.isAnnotation() -> devstudioai.lang.resolve.SymbolKind.ANNOTATION_TYPE
                    else -> devstudioai.lang.resolve.SymbolKind.CLASS
                }
                addStructure(decl.name, null, kind, decl, depth, out)
                for (m in decl.declarations) collectStructure(m, depth + 1, out)
            }
            is KtNamedFunction -> addStructure(decl.name, "(${paramTypes(decl.valueParameters)})", devstudioai.lang.resolve.SymbolKind.METHOD, decl, depth, out)
            is KtSecondaryConstructor -> addStructure("constructor", "(${paramTypes(decl.valueParameters)})", devstudioai.lang.resolve.SymbolKind.CONSTRUCTOR, decl, depth, out)
            is KtProperty -> addStructure(decl.name, decl.typeReference?.text, devstudioai.lang.resolve.SymbolKind.FIELD, decl, depth, out)
            else -> {}
        }
    }
    ...
    private fun addStructure(name: String?, detail: String?, kind: devstudioai.lang.resolve.SymbolKind, element: KtElement, depth: Int, out: MutableList<devstudioai.lang.resolve.StructureItem>) {
        val n = name ?: return
        val nameOffset = (element as? KtNamedDeclaration)?.nameIdentifier?.textRange?.startOffset ?: element.textRange.startOffset
        out.add(devstudioai.lang.resolve.StructureItem(n, detail, kind, nameOffset, element.textRange.endOffset, depth))
    }
    ...
    override fun quickDoc(file: VirtualFile, text: CharSequence, offset: Int): devstudioai.lang.resolve.QuickDocInfo? {
        val ktFile = KotlinParserHost.parse(file.name, text)
    ...
    private fun symbolDoc(sym: KotlinSymbol): devstudioai.lang.resolve.QuickDocInfo {
        ...
        val (doc, fmt) = if (rawKdoc != null) rawKdoc to devstudioai.lang.resolve.DocFormat.KDOC
        else sym.documentation() to devstudioai.lang.resolve.DocFormat.PLAIN
        return devstudioai.lang.resolve.QuickDocInfo(sig, sym.name, sym.kind, container, doc, fmt)
    }

    private fun declarationDoc(decl: KtNamedDeclaration): devstudioai.lang.resolve.QuickDocInfo {
        val sig: String
        val kind: devstudioai.lang.resolve.SymbolKind
        when (decl) {
            is KtNamedFunction -> {
                sig = "fun ${decl.name}(${paramTypes(decl.valueParameters)})" + (decl.typeReference?.text?.let { ": $it" } ?: "")
                kind = devstudioai.lang.resolve.SymbolKind.METHOD
            }
            is KtProperty -> {
                sig = "${if (decl.isVar) "var" else "val"} ${decl.name}" + (decl.typeReference?.text?.let { ": $it" } ?: "")
                kind = devstudioai.lang.resolve.SymbolKind.FIELD
            }
            is KtClass -> {
                sig = "${if (decl.isInterface()) "interface" else "class"} ${decl.name}"
                kind = if (decl.isInterface()) devstudioai.lang.resolve.SymbolKind.INTERFACE else devstudioai.lang.resolve.SymbolKind.CLASS
            }
            else -> { sig = decl.name ?: ""; kind = devstudioai.lang.resolve.SymbolKind.CLASS }
        }
        ...
        return devstudioai.lang.resolve.QuickDocInfo(
            sig, decl.name ?: "", kind, container, raw,
            if (raw != null) devstudioai.lang.resolve.DocFormat.KDOC else devstudioai.lang.resolve.DocFormat.PLAIN,
        )
    }
