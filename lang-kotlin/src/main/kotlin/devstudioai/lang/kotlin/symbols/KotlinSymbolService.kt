...
import devstudioai.lang.synthetic.SyntheticMethod
import devstudioai.lang.synthetic.SyntheticModifier
import devstudioai.lang.synthetic.SyntheticTypeKind
...
class KotlinSymbolService(
    ...
    private val sourceDoc: devstudioai.lang.resolve.SourceDocProvider = devstudioai.lang.resolve.SourceDocProvider.NONE,
    ...
    fun membersForCompletion(typeFqn: String, typeArgs: List<TypeRef>, namePrefix: String): List<KotlinSymbol> {
        val own = devstudioai.lang.kotlin.KotlinPerf.span("own") { ownAndInherited(typeFqn, typeArgs, HashSet()) }
        val ownMatched = if (namePrefix.isEmpty()) own else own.filter { it.name.startsWith(namePrefix, ignoreCase = true) }
        return ownMatched + devstudioai.lang.kotlin.KotlinPerf.span("ext") { extensionsFor(typeFqn, typeArgs, namePrefix) }
    }
    ...
    fun extensionsFor(fqnRaw: String, typeArgs: List<TypeRef> = emptyList(), namePrefix: String = ""): List<KotlinSymbol> {
        ...
        val fromClasspath = if (idx != null) {
            ...
            if (!idx.status.ready) emptyList() // dumb: no index yet → no classpath extensions (no scan)
                ...
                devstudioai.lang.kotlin.KotlinPerf.span("ext.index") { targets.flatMap { t ->
    ...
    private fun enrich(s: KotlinSymbol): KotlinSymbol {
        if (sourceDoc === devstudioai.lang.resolve.SourceDocProvider.NONE || s.origin.fromSource) return s
    ...
    private fun toSymbol(rc: RawCallable, ownerFqn: String?, ownerTypeParams: List<String> = emptyList()): KotlinSymbol {
        ...
        return KotlinSymbol(
            ...
            modifiers = when (rc.visibility) {
                "private" -> setOf(devstudioai.lang.resolve.Modifier.PRIVATE)
                "protected" -> setOf(devstudioai.lang.resolve.Modifier.PROTECTED)
                else -> emptySet()
            } + if (rc.isAbstract) setOf(devstudioai.lang.resolve.Modifier.ABSTRACT) else emptySet(),
    ...
    private fun inferReturnFromBody(rc: RawCallable): KotlinType? {
        val dom = rc.node as? devstudioai.lang.kotlin.parse.KotlinDomNode ?: return null
        ...
        val result = try {
            devstudioai.lang.kotlin.resolve.KotlinResolver(dom.owner.ktFile, dom.owner, this).inferType(body)
        } catch (t: Throwable) {
            null
        } finally {
            guard.remove(rc)
