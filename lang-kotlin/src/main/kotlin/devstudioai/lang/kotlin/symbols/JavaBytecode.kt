...
object JavaBytecode {
        ...
        cr.accept(object : ClassVisitor(Opcodes.ASM9) {
            override fun visit(version: Int, access: Int, name: String?, sig: String?, superName: String?, interfaces: Array<out String>?) {
                ...
                if (sig != null) {
                    ...
                    interfaces?.forEach { superTypes += KotlinType(it.replace('/', '.'), context = ctx) }
                }
            }
            ...
            override fun visitMethod(access: Int, name: String, descriptor: String, sig: String?, exceptions: Array<out String>?): MethodVisitor? {
                ...
                val displayParams = t.argumentTypes.mapIndexed { i, at -> "p$i: ${at.className.substringAfterLast('.')}" }
                val classFqn = selfName?.replace('/', '.')?.replace('$', '.')
                val display = if (isCtor) "(${displayParams.joinToString(", ")})"
                else "(${displayParams.joinToString(", ")}): ${t.returnType.className.substringAfterLast('.')}"
                val (retType, paramTypes, methodTypeParams, methodBounds) =
                    if (sig != null) parseMethodSignature(sig, ctx)
                    else MethodTypes(
    ...
    private abstract class FormalsVisitor(protected val ctx: KotlinTypeContext?) : SignatureVisitor(Opcodes.ASM9) {
    ...
    private class MethodSigVisitor(ctx: KotlinTypeContext?) : FormalsVisitor(ctx) {
        ...
        override fun visitParameterType(): SignatureVisitor = TypeSigVisitor(ctx) { paramTypes += it }
    ...
    private class TypeSigVisitor(
        ...
        override fun visitTypeVariable(name: String) = sink(KotlinType(name, isTypeParameter = true, context = ctx))

        override fun visitArrayType(): SignatureVisitor =
            TypeSigVisitor(ctx) { el -> sink(KotlinType("kotlin.Array", listOf(el), context = ctx)) }

        override fun visitClassType(name: String) { classFqn = name.replace('/', '.') }

        override fun visitInnerClassType(name: String) { classFqn = (classFqn ?: "") + "." + name }

        override fun visitTypeArgument() { args += KotlinType("kotlin.Any", context = ctx) } // unbounded `?`
