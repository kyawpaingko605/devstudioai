 ...
 * the ASM patch passes in `buildSrc` (`devstudioai.build.kotlinc.ArtPatchPasses`): each failure this surfaces
 ...
 *
 * Because [devstudioai.build.kotlinc.ArtPatchPasses] starts empty, the first run is expected to fail; that
...
class KotlinCompilerArtSpikeTest {
        ...
        val exit: ExitCode = try {
            ...
            fail(
                "Kotlin compiler failed to run on ART: ${t.javaClass.name}: ${t.message}\n" +
                    "Add a pass to devstudioai.build.kotlinc.ArtPatchPasses targeting the class in this trace, " +
    ...
    fun composeCompilesOnArt() {
        ...
        val pluginJar = devstudioai.lang.kotlin.compile.ComposeCompilerPlugin.jar()
     ...
     *
     *     ./gradlew :ide-android:connectedDebugAndroidTest \
     *       -Pandroid.testInstrumentationRunnerArguments.class=devstudioai.android.spike.KotlinCompilerArtSpikeTest#kotlinCompileTimingOnArt
     *     adb logcat -s KotlincArtSpike
     */
    @Test
     ...
     * ([devstudioai.lang.kotlin.compile.KotlinJvmCompiler.warmUp] — what `IdeServices` now schedules off-thread at
    ...
    fun kotlinWarmUpFrontLoadsColdStartOnArt() {
        ...
        val compiler = devstudioai.lang.kotlin.compile.KotlinJvmCompiler()
