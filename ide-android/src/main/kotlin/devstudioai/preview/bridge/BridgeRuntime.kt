package devstudioai.preview.bridge

import android.content.Context
import android.content.res.TypedArray
import android.util.AttributeSet
import devstudioai.preview.NativeCanvasHolder
import devstudioai.preview.RCanvas
import devstudioai.preview.ResolvedValue
import devstudioai.preview.ViewHost
import devstudioai.preview.impl.StyledAttrResolver

/**
 * The on-device Bridge runtime the BridgeRemapper reparents user views onto. The instrumented user class
 * `extends devstudioai.preview.bridge.widget.BridgeView` (a real `android.view.View`), and its
 * `obtainStyledAttributes(...)` call is redirected to [Bridges.obtainStyledAttributes], which returns a
 * [devstudioai.preview.bridge.res.BridgeTypedArray] of pre-resolved values. The view renders into our owned
 * canvas via [ViewHost], unwrapping the native `android.graphics.Canvas` for the user's `onDraw`.
 *
 * Package/class names here MUST match `devstudioai.preview.impl.bridge.BridgeTypeMap` exactly.
 */
...
object Bridges {
    ...
    @JvmStatic
    fun obtainStyledAttributes(context: Context?, set: AttributeSet?, attrs: IntArray, defStyleAttr: Int, defStyleRes: Int): devstudioai.preview.bridge.res.BridgeTypedArray {
        val raw = (set as? BridgeAttributeSet)?.rawAttrs ?: emptyMap()
        val values = styledResolver.get()?.resolve(raw, attrs) ?: emptyList()
        return devstudioai.preview.bridge.res.BridgeTypedArray(values)
    }
}
...
/**
 * Adapts a reparented [devstudioai.preview.bridge.widget.BridgeView] (a real `View`) to the engine's [ViewHost]:
 ...
 */
class AndroidViewHost(private val view: devstudioai.preview.bridge.widget.BridgeView) : ViewHost {
