package devstudioai.android

import android.content.Context
import android.os.Build
import devstudioai.analytics.DeviceInfo
import devstudioai.analytics.impl.AnalyticsLogSink
import devstudioai.analytics.impl.DefaultAnalyticsService
import devstudioai.analytics.impl.SupabaseSink
import devstudioai.core.IdeServicesBackend
import devstudioai.core.ProjectManager
import devstudioai.platform.log.Log.addSink
import java.io.File
...
object AndroidIde {
    ...
    fun bootstrap(context: Context): Session {
        ...
        val backend = IdeServicesBackend(
            ...
            buildRunnerFactory = { svc -> devstudioai.android.daemon.RemoteBuildRunner(appContext, svc) },
        )
        ...
        if (backend.diagnostics.analyticsConsent() == true) {
            backend.diagnostics.track(devstudioai.analytics.Events.COLD_START, mapOf("duration_ms" to ((System.nanoTime() - startNs) / 1_000_000).toString()))
        }
    ...
    fun createProjectManager(context: Context): ProjectManager {
        ...
        // PathUtil reads (see build-logic devstudioai.build.kotlinc.PathUtilSelfLocatePass). Without this, the
        ...
        val previewRuntime = devstudioai.preview.bridge.DexCustomViewRuntime(
        ...
        val settingsPrefix = "settings.${devstudioai.core.settings.BuiltInSettingsPages.BUILD_RUNTIME}."
        val r8HeapKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.R8_MAX_HEAP
        val r8ModeKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.R8_MODE
        ...
        val r8HeapProvider = {
            ...
            mgr?.preference(r8HeapKey)?.trim()?.toIntOrNull()
                ?: mgr?.preference(devstudioai.core.settings.BuiltInSettingsPages.R8_CEILING_PREF)?.trim()?.toIntOrNull()?.takeIf { it > 0 }
        }
        ...
        val dexOffHeapKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.DEX_OFFHEAP_MB
        val dexOffHeapProvider = { managerRef.get()?.preference(dexOffHeapKey)?.trim()?.toIntOrNull() }
        val dexMergeBatchKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.DEX_MERGE_BATCH
        val dexMergeChunkProvider = {
            managerRef.get()?.preference(dexMergeBatchKey)?.trim()?.toIntOrNull()?.takeIf { it > 0 }
                ?: devstudioai.core.settings.BuiltInSettingsPages.DEX_MERGE_BATCH_DEFAULT
        }
        ...
        val dexForkConcurrencyKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.DEX_FORK_CONCURRENCY
        ...
        val separateProcessKey = settingsPrefix + devstudioai.core.settings.BuiltInSettingsPages.SEPARATE_PROCESS
        val realViewRuntime = devstudioai.preview.realview.RemoteRealViewRuntime(
     ...
     */
    private fun buildAnalytics(manager: ProjectManager, home: File): devstudioai.analytics.AnalyticsService {
        val url = BuildConfig.ANALYTICS_URL
        val key = BuildConfig.ANALYTICS_KEY
        if (url.isBlank() || key.isBlank()) return devstudioai.analytics.NoopAnalyticsService
    ...
    private fun detectR8CeilingAsync(context: Context, manager: ProjectManager) {
        ...
        Thread {
            runCatching {
                ...
                manager.setPreference(devstudioai.core.settings.BuiltInSettingsPages.R8_CEILING_PREF, ceiling.toString())
