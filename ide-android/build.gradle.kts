...
import devstudioai.build.RelocateTypesInJar
...
plugins {
    ...
    // ALL — see buildSrc devstudioai.build.kotlinc). No-op until the device spike (KotlinCompilerArtSpikeTest)
    // discovers the first breakage and a pass is added to ArtPatchPasses.
    id("devstudioai.kotlinc-art")
}
...
// (devstudioai.lang.jdt.compat.RuntimeVersion in :lang-jdt), then dex the patched jar instead of the
...
// ship (devstudioai.lang.jdt.compat.StackWalker in :lang-jdt) and dex the patched jars. These two modules
...
// standalone r8.dex.zip asset; `devstudioai.android.ForkedR8Shrinker` extracts it and runs
...
// NOTE: the `devstudioai.ui.generated.resources` segment must match :ide-ui's `packageOfResClass`.
val bundleComposeFontsAsset = tasks.register<Copy>("bundleComposeFontsAsset") {
    ...
    into(layout.buildDirectory.dir("compose-fonts-asset/composeResources/devstudioai.ui.generated.resources/font"))
}

android {
    namespace = "devstudioai.android"
    compileSdk = 36
    compileSdkMinor = 1

    defaultConfig {
        // The Play Store identity (immutable once published). The Kotlin source package (namespace,
        // above) stays devstudioai.android — that's an internal build detail, independent of applicationId.
        applicationId = "devstudioai"
...
val bundleComposeStringAsset = tasks.register<Copy>("bundleComposeStringAsset") {
    ...
    from(project(":ide-ui").layout.buildDirectory.dir("processedResources/desktop/main/composeResources/devstudioai.ui.generated.resources")) {
        include("values*/**/*.cvr")
    }
    into(layout.buildDirectory.dir("compose-strings-asset/composeResources/devstudioai.ui.generated.resources"))
}
...
dependencies {
    ...
    // devstudioai.interp.compose — KMP, re-exporting :interp-core): drives a ResolvedTree against the real
