...
import devstudioai.lang.AnnotationProcessor
import devstudioai.lang.CompilationContext
import devstudioai.lang.completion.CompletionRequest
import devstudioai.lang.completion.complete
...
fun signatureHelpAt(analyzer: JdtSourceAnalyzer, file: Path, codeWithCaret: String): devstudioai.lang.signature.SignatureHelp? {
    val offset = codeWithCaret.indexOf("|CARET|")
    require(offset >= 0) { "missing |CARET|" }
    val text = codeWithCaret.replace("|CARET|", "")
    val request = devstudioai.lang.signature.SignatureHelpRequest(
        Snapshot(StubFile(file.toString(), text), 1, text), offset, devstudioai.lang.signature.SignatureHelpTrigger.Explicit,
    )
